...
namespace DXUtils
{
        ...
        fastnoise::TLogFn logFn)
    {
        ...
        if (FAILED(hr))
        {
            if (error)
            {
                ...
                logFn((int)fastnoise::LogLevel::Error, "Could not compile shader %ls:\n%s", shaderFile, errorMsg);
                error->Release();
                error = nullptr;
            }
            else
            {
                logFn((int)fastnoise::LogLevel::Error, "Could not compile shader %ls", shaderFile);
            }
            return nullptr;
        }
        else if (error)
        {
            const char* errorMsg = (const char*)error->GetBufferPointer();
            logFn((int)fastnoise::LogLevel::Warn, "Shader compilation warning %ls:\n%s", shaderFile, errorMsg);
            error->Release();
            error = nullptr;
        }
        ...
        fastnoise::TLogFn logFn)
    {
        ...
        if (FAILED(hr))
        {
            logFn((int)fastnoise::LogLevel::Error, "Could not create PSO for shader %ls", shaderFile);
            return false;
        }
