    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Compute Shader: Initialise
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Texture, context->m_output.texture_Texture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture3D, false, 0, context->m_output.texture_Texture_size[2] },
                { context->m_output.buffer_Data, context->m_output.buffer_Data_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_Data_stride, context->m_output.buffer_Data_count },
                { context->m_input.buffer_InitBuffer, context->m_input.buffer_InitBuffer_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Buffer, false, context->m_input.buffer_InitBuffer_stride, context->m_input.buffer_InitBuffer_count },
        ...
        // Compute Shader: CalculateLoss
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_internal.texture_Loss, context->m_internal.texture_Loss_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture3D, false, 0, context->m_internal.texture_Loss_size[2] },
                { context->m_input.buffer_Filter, context->m_input.buffer_Filter_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Buffer, false, context->m_input.buffer_Filter_stride, context->m_input.buffer_Filter_count },
                { context->m_output.texture_Texture, context->m_output.texture_Texture_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Texture3D, false, 0, context->m_output.texture_Texture_size[2] },
                { context->m_internal.constantBuffer__LossCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
        ...
        // Compute Shader: Swap
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_internal.texture_Loss, context->m_internal.texture_Loss_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Texture3D, false, 0, context->m_internal.texture_Loss_size[2] },
                { context->m_output.texture_Texture, context->m_output.texture_Texture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture3D, false, 0, context->m_output.texture_Texture_size[2] },
                { context->m_output.texture_SwapDebug, context->m_output.texture_SwapDebug_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture3D, false, 0, context->m_output.texture_SwapDebug_size[2] },
                { context->m_output.buffer_Data, context->m_output.buffer_Data_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_Data_stride, context->m_output.buffer_Data_count },
                { context->m_internal.constantBuffer__SwapCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Texture
        {
               ...
               m_output.texture_Texture_format != desiredFormat)
            {
                ...
                m_output.texture_Texture = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_Texture_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture3D, (c_debugNames ? L"Texture" : nullptr), Context::LogFn);
                m_output.texture_Texture_size[0] = desiredSize[0];
                m_output.texture_Texture_size[1] = desiredSize[1];
                m_output.texture_Texture_size[2] = desiredSize[2];
                m_output.texture_Texture_format = desiredFormat;
        ...
        // For storing values of the loss function
        {
               ...
               m_internal.texture_Loss_format != desiredFormat)
            {
                ...
                m_internal.texture_Loss = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_internal.texture_Loss_flags, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, DX12Utils::ResourceType::Texture3D, (c_debugNames ? L"Loss" : nullptr), Context::LogFn);
                m_internal.texture_Loss_size[0] = desiredSize[0];
                m_internal.texture_Loss_size[1] = desiredSize[1];
                m_internal.texture_Loss_size[2] = desiredSize[2];
                m_internal.texture_Loss_format = desiredFormat;
        ...
        // SwapDebug
        {
               ...
               m_output.texture_SwapDebug_format != desiredFormat)
            {
                ...
                m_output.texture_SwapDebug = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_SwapDebug_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture3D, (c_debugNames ? L"SwapDebug" : nullptr), Context::LogFn);
                m_output.texture_SwapDebug_size[0] = desiredSize[0];
                m_output.texture_SwapDebug_size[1] = desiredSize[1];
                m_output.texture_SwapDebug_size[2] = desiredSize[2];
                m_output.texture_SwapDebug_format = desiredFormat;
